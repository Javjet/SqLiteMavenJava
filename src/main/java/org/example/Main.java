...
public class Main {
    private static String url = "jdbc:sqlite:src/main/resources/BIBLIOTECA.dat";
    private static String user ="root";
    private static String password ="alumno";

    //static String  createDatabaseSQL="CREATE DATABASE IF NOT EXISTS CENTROS_HUESCA";
    static String[] createTableSQLqueries = {
        "CREATE TABLE IF NOT EXISTS C1_ASIGNATURAS(COD_ASIG CHAR(6) NOT NULL PRIMARY KEY,NOMBRE_ASI VARCHAR(30));",

        "CREATE TABLE IF NOT EXISTS C1_CENTROS(COD_CENTRO NUMERIC(4) NOT NULL PRIMARY KEY ,NOM_CENTRO VARCHAR(20),DIRECTOR NUMERIC(4),DIRECCION VARCHAR(25),LOCALIDAD VARCHAR(20),PROVINCIA VARCHAR(20));",

        "CREATE TABLE IF NOT EXISTS C1_ESPECIALIDAD(ESPECIALIDAD CHAR(2) NOT NULL PRIMARY KEY,NOMBRE_ESPE VARCHAR(25));",

        "CREATE TABLE IF NOT EXISTS C1_PROFESORES(COD_PROF NUMERIC(4) NOT NULL PRIMARY KEY,NOMBRE_APE VARCHAR(30),ESPECIALIDAD CHAR(2) REFERENCES C1_ESPECIALIDAD(ESPECIALIDAD ),JEFE_DEP NUMERIC(4), FECHA_NAC DATE, SEXO CHAR(1), COD_CENTRO NUMERIC(4) NOT NULL REFERENCES C1_CENTROS (COD_CENTRO)) ;",

        "CREATE TABLE IF NOT EXISTS  C1_ASIGPROF(COD_ASIG CHAR(6) NOT NULL REFERENCES C1_ASIGNATURAS (COD_ASIG),COD_PROF NUMERIC(4) NOT NULL REFERENCES C1_PROFESORES (COD_PROF),PRIMARY KEY (COD_ASIG,COD_PROF)) ;"
    };

    static String[] insertRegisters = {
    ...
    public static void main(String[] args) {
        url = "jdbc:sqlite:src/main/resources/CENTROS_HUESCA.dat";
        File fichero=new File("src/main/resources/CENTROS_HUESCA.dat");
        if(fichero.exists()){
            fichero.delete();
        }
        try(Connection conexion= DriverManager.getConnection(url,user,password)) {
            //System.out.println(crearDataBase(conexion,createDatabaseSQL));
            //conexion.createStatement().execute("USE CENTROS_HUESCA");
            crearTablas(conexion,createTableSQLqueries);
            añadirRegistros(conexion,insertRegisters);
            /*ResultSet rs= conexion.createStatement().executeQuery("SELECT ESPECIALIDAD FROM C1_ESPECIALIDAD");
            while (rs.next()){
    ...
    public static void crearTabla (Connection conexion, String SentenciaSql) throws SQLException {
        conexion.createStatement().execute(SentenciaSql);
    }
    public static void crearTablas (Connection conexion, String[] sentenciasSqlTablas) throws SQLException {
        Statement s = conexion.createStatement();
        for (String cadenaSql:sentenciasSqlTablas) {
            s.execute(cadenaSql);
        }

    }
    public static void añadirRegistros(Connection conexion, String[] sentenciasSqlTablas) throws SQLException {
        Statement s = conexion.createStatement();
        for (String cadenaSql:sentenciasSqlTablas) {
            s.execute(cadenaSql);
        }
    }
    public static String crearDataBase(Connection conexion, String sentenciaSqlCrearBase) throws SQLException {
        conexion.createStatement().execute(sentenciaSqlCrearBase);
        return sentenciaSqlCrearBase.substring(sentenciaSqlCrearBase.lastIndexOf(" "));
    }
}